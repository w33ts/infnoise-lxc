...
2. Pick a new semantic version tag such as `v0.1.0`.
3. Create and push the tag.
4. Wait for the `Release` GitHub Actions workflow to finish.
5. Confirm the GitHub Release contains the tarball and checksum assets.
6. Test the Proxmox host install flow with the new tag.
...
- `infnoise-lxc-v0.1.0.tar.gz`
- `infnoise-lxc-v0.1.0.sha256`

You can also verify the checksum after downloading:

```bash
sha256sum -c infnoise-lxc-v0.1.0.sha256
```
